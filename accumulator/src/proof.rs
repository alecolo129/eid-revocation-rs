...
impl Proof {
    ...
    pub fn finalize(&self, params: &ProofParamsPublic) -> ProofFinal {
        // e(A_bar, X_2) - e(B_bar, g_2) = 0
        let pair_final = multi_miller_loop(&[
            (
                &self.a_bar.to_affine(),
                &G2Prepared::from(params.x_2.to_affine()),
            ),
            (
                &(-self.b_bar).to_affine(),
                &G2Prepared::from(params.g_2.to_affine()),
            ),
        ])
...
impl ProofFinal {
    ...
    pub fn verify(&self, transcript: &mut Transcript) -> bool {
        ...
        return (self.pair_final.is_identity().unwrap_u8() == 1)
            && self.challenge_hash == computed_challenge.0;
    }
}
...
mod tests {
    ...
    use crate::{
        accumulator::Element, proof::Proof, Accumulator, MembershipWitness, ProofCommitting,
        ProofParamsPrivate, ProofParamsPublic, PublicKey, SecretKey, PROOF_LABEL,
    };
