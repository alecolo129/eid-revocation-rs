...
mod tests {
    ...
    fn key_coefficient_test() {
        ...
        let t1 = Instant::now();
        let ago_key = ago_acc::SecretKey(key.0);
        let ago_data: Vec<_> = data.iter().map(|d| ago_acc::Element(d.0)).collect();
        let coefficients = ago_key.create_coefficients(&[], &ago_data);
        let t1 = t1.elapsed();
        ...
        println!("Agora Implementation: coefficient generation: {:?}", t1);
