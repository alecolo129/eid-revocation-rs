...
use network::{base_registry, server, log_with_time, log_with_time_ln, controller::Controller, client::Client};
...
async fn main() {
    ...
    assert!(Client::poly_upd(&mut holders[non_revoked_index]).await.unwrap());
    assert!(holders[non_revoked_index].test_membership(None));


    // Batch revoke half of the clients
    let revoked = holders[1..NUM_CLIENTS/2].iter().map(|h| h.get_pseudo()).collect();
    assert!(Controller::revoke_batch_now(&revoked).await.expect("Cannot revoke batch").status().is_success());
    ...
    let revoked = holders[NUM_CLIENTS/2..non_revoked_index].iter().map(|h| h.get_pseudo()).collect();
    assert!(Controller::revoke_batch(&revoked).await.expect("Cannot revoke batch").status().is_success());


    // Issuer performs periodic update
    Controller::update_periodic_and_send().await.expect("Cannot perform periodc update");

    // Non-revoked client gets periodic witness update which includes revocation of all remaining holders
    assert!(Client::wit_upd(&mut holders[non_revoked_index]).await.expect("Cannot get updated witness"));
    ...
    for hol in holders[0..non_revoked_index].iter_mut() {
        assert!(!Client::wit_upd(hol).await.expect("Cannot get updated witness"));
    ...
    log_with_time!(
        "Creating and verifying membership proof for non-revoked client ...",
    );
    assert!(ver.verify(holders[non_revoked_index].proof_membership(None)));
